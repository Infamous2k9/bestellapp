...
import { localStorageHelper } from "../utils/utlis";
...
export const cartStore = {
    init() {
        ...
        if (savedCart && Array.isArray(savedCart.cartItems)) {
            ...
            cart.deliveryType = savedCart.deliveryType ?? 'takeOut';
        } else {
            cart.cartItems = [];
            cart.deliveryType = 'takeOut';
            this.save();
        }
    ...
    findDishById(dishId) {
        ...
        return dishes.find((dish) => String(dish.id) === String(dishId));
    ...
    recalculateTotals() {
        ...
        const baseDeliveryFee = 4.99;


        if (cart.deliveryType === 'takeIn') {
            cart.deliveryFee = 0;
            return;
        }
